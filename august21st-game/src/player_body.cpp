...
void PlayerBody::_bind_methods()
{
	...
	ClassDB::bind_method(D_METHOD("_on_stats_level_changed", "level"),
		&PlayerBody::_on_stats_level_changed);
}
...
void PlayerBody::_ready()
{
	...
	_client->connect("stats_changed", Callable(this, "_on_stats_level_changed"));
	...
	_stats_label->set_visible(false);
	_stats_level = 0;
		...
		if (is_on_floor()) {
			if (_velocity.y < 0) {
				...
				if (fall_speed > FALL_DAMAGE_THRESHOLD) {
					// (x=_velocity.y, y=fall_damage), a=FALL_DAMAGE_THRESHOLD, d=20.0, b=FALL_DAMAGE_MIN
					// f\left(x\right)=\frac{d-b}{\left(1-a\right)^{2}}\left(x-a\right)^{2}+b
					auto fall_damage = (20.0 - FALL_DAMAGE_MIN) / Math::pow(FALL_DAMAGE_THRESHOLD, 2.0f)
						* Math::pow(fall_speed - FALL_DAMAGE_THRESHOLD, 2.0f)
						+ FALL_DAMAGE_MIN;
...
void PlayerBody::_on_stats_level_changed(int level)
{
	_stats_level = level;
	_stats_label->set_visible(level > 0);
}

void PlayerBody::update_stats()
{
	auto stats_string = String(
		"PlayerBody:\nInternal velocity: {0}\nVelocity: {1}"
			"\nPosition: {2},\nRotation:{3}")
		.format(Array::make(_velocity, get_velocity(), get_position(), get_rotation()));
	_stats_label->set_text(stats_string);
}

void PlayerBody::_process(double delta)
{
	// Update stats if stats enabled on client
	if (_stats_level > 0) {
		update_stats();
	}
...
void PlayerBody::_on_revive_pressed()
{
    ...
    respawn_packet.u8(to_uint8(ClientPacket::ACTION_RESPAWN));
    _client->send(respawn_packet);
}
...
void PlayerBody::_on_packet_received(PackedByteArray packed_packet)
{
	...
	switch (code) {
		case ServerPacket::CHAT_MESSAGE: {
			...
			String chat_name;
			if (player_id == 0) {
				chat_name = "SERVER@AUGUST21ST-GAME";
			}
			else {
