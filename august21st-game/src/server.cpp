		...
		while (_socket_server->get_available_packet_count()) {
			...
			switch (code) {
				...
				case ClientPacket::ACTION_RESPAWN: {
					// TODO: Use scene / player respawn position
					auto respawn_position = Vector3(0, 0, 0);
					auto respawn_packet = BufWriter();
					respawn_packet.u8(to_uint8(ServerPacket::RESPAWN));
					respawn_packet.u32(sender_id);
					write_vector3(respawn_packet, respawn_position);
					send_to_authenticated(respawn_packet);
					break;
				}
